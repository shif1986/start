  ...
  return (
    <AccountShell eyebrow={content.eyebrow} title={content.title} description={content.description} navigation={content.nav}>
      ...
      <div className="mt-8 grid grid-cols-[1.35fr_.65fr] gap-6 max-lg:grid-cols-1">
        <section className="rounded-2xl border border-start-cream/10 bg-[#121418] p-6 max-sm:p-5"><h2 className="text-xl font-semibold">Activité récente</h2><div className="mt-5 divide-y divide-start-cream/10">{mockListings.slice(0, 3).map((listing) => <div key={listing.id} className="flex items-center justify-between gap-4 py-4 max-sm:flex-col max-sm:items-start max-sm:gap-2"><div className="min-w-0"><strong className="block break-words">{listing.title}</strong><span className="text-sm text-start-cream/45">{listing.city} · {listing.category}</span></div><span className="shrink-0 rounded-full border border-start-gold/25 px-3 py-1 text-xs text-start-gold">Actif</span></div>)}</div></section>
