...
import { signInWithEmail, signInWithGoogle, signUpWithEmail } from "../features/auth/api/auth-actions";
...
export default function AuthPage({ mode }: { mode: "login" | "register" }) {
  ...
  async function handleEmailSubmit(values: AuthFormValues) {
    ...
    try {
      ...
      navigate(redirect && (!isRegister || accountType === "professional") ? redirect : accountHome, { replace: true });
    } catch (error) {
  ...
  async function handleGoogle() {
    ...
    try {
      await signInWithGoogle(redirect ?? accountHome);
    } catch (error) {
