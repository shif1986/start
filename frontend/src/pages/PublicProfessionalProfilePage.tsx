...
export default function PublicProfessionalProfilePage() {
  ...
  const profileQuery = usePublicProfessionalProfile(username, true, isSupabase && Boolean(session));
  ...
  if (!session) return <ThemedPage ambiance="network" className="grid min-h-[55vh] place-items-center p-[clamp(24px,5vw,72px)]"><section className="max-w-lg rounded-2xl border border-start-gold/25 bg-[#121418] p-8 text-center"><span className="text-xs font-bold tracking-[.2em] text-start-gold uppercase">Profil privé</span><h1 className="mt-3 text-2xl font-semibold">Identité réservée aux membres</h1><p className="mt-4 leading-7 text-start-cream/60">Connectez-vous ou créez un compte gratuit pour consulter l’identité, les annonces et les coordonnées autorisées de ce membre.</p><Link className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-start-gold px-6 font-bold text-start-ink" to={`/connexion?redirect=${encodeURIComponent(`/professionnel/${username}`)}`}>Se connecter</Link></section></ThemedPage>;
  ...
  return <ThemedPage ambiance="network" className="p-[clamp(20px,5vw,72px)]">
    <header className="grid grid-cols-[auto_1fr] items-center gap-6 rounded-2xl border border-start-cream/10 bg-[#121418] p-[clamp(20px,4vw,44px)] max-sm:grid-cols-1">
      {profile.avatarUrl ? <img src={profile.avatarUrl} alt="" className="size-28 rounded-full object-cover" /> : <span className="grid size-28 place-content-center rounded-full bg-start-gold/15 text-4xl font-bold text-start-gold" aria-hidden="true">{(profile.companyName ?? profile.displayName).slice(0, 1).toUpperCase()}</span>}
      <div><span className="text-xs font-bold tracking-[.2em] text-start-gold uppercase">Professionnel {profile.isVerified ? "vérifié" : "du réseau"}</span><h1 className="mt-2 text-[clamp(1.8rem,4vw,3.4rem)]">{profile.companyName ?? profile.displayName}</h1>{profile.companyName && <p className="mt-2 font-semibold text-start-cream/75">{profile.displayName}</p>}<p className="mt-1 text-start-cream/55">@{profile.username}{profile.city ? ` · ${profile.city}` : ""}</p>{profile.bio && <p className="mt-5 max-w-3xl leading-7 text-start-cream/70">{profile.bio}</p>}</div>
    </header>
    <section className="mt-8 rounded-2xl border border-start-cream/10 bg-[#121418] p-6"><h2 className="text-xl font-semibold">Coordonnées</h2>{profile.contacts && (profile.contacts.phone || profile.contacts.publicEmail || profile.contacts.postalAddress) ? <ul className="mt-4 grid gap-2 text-start-cream/70">{profile.contacts.phone && <li><a className="text-start-gold underline" href={`tel:${profile.contacts.phone}`}>{profile.contacts.phone}</a></li>}{profile.contacts.publicEmail && <li><a className="text-start-gold underline" href={`mailto:${profile.contacts.publicEmail}`}>{profile.contacts.publicEmail}</a></li>}{profile.contacts.postalAddress && <li>{profile.contacts.postalAddress}</li>}</ul> : <p className="mt-3 text-sm text-start-cream/55">Aucune coordonnée disponible selon vos droits actuels.</p>}</section>
