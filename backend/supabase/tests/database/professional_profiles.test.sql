...
values
  ('a1000000-0000-4000-8000-000000000001', 'profile-pro@example.test', '{"display_name":"Jean Dupont","company_name":"Impact Conseil","account_type":"professional"}'::jsonb),
  ...
  ('a1000000-0000-4000-8000-000000000004', 'profile-google@example.test', '{"full_name":"Nom Google"}'::jsonb);

select is((select display_name from public.profiles where id = 'a1000000-0000-4000-8000-000000000004'), 'Nom Google', 'le nom Google est utilisé avant le préfixe e-mail');
select is((select company_name from public.profiles where id = 'a1000000-0000-4000-8000-000000000001'), 'Impact Conseil', 'le nom d’entreprise est créé depuis l’inscription');
...
set local role anon;
select is((select count(*) from public.active_professional_profiles), 1::bigint, 'seuls les professionnels actifs possèdent un profil consultable');
select is((select display_name from public.active_professional_profiles limit 1), null::text, 'le nom du professionnel est masqué au visiteur');
...
select lives_ok(
  $$select public.update_my_profile(
    'profil-pro-public', 'Jean Dupont', 'Impact Conseil France', 'https://example.test/avatar.webp',
...
select is((select company_name from public.profiles where id = auth.uid()), 'Impact Conseil France', 'le professionnel peut modifier son entreprise séparément');
...
select throws_ok(
  $$select public.update_my_profile('profil-suspendu', 'Profil suspendu', null, null, null, null, null, null, null)$$,
